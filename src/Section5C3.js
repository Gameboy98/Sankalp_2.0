...
export default class Section5C2 extends Component {
  constructor(props) {
    super(props);

    const totalCost5C1 = this.props.navigation.getParam('totalCost5C1', '');
    const totalCost5C2 = this.props.navigation.getParam('totalCost5C2', '');
    this.state = {
      resourcePosition: '',
      costAnnum:'',
      units: '',
      ...
      totalCost5C1:totalCost5C1,
      totalCost5C2:totalCost5C2,
  ...
  CheckTextInput = () => {
    if (this.state.hardwareCost == '' || this.state.districtGap == '') {
      alert('Please Enter Fields');
    }else {

      this.props.navigation.navigate("Section5F");
    }
  ...
  render() {
    var f1 = this.state.hardwareCost;
      var f2 = this.state.districtGap;
      var f3 = this.state.totalCost5C1 ;
      var f4 = this.state.totalCost5C2 ;
          const result = f1 && f2 ?  Number(f1) + Number(f2)+Number(f3) + Number(f4) : null;
    return (
          ...
          <View>
            <View style={styles.question}>
              <View style={styles.check1}>
                ...
                <Text style={styles.text2}>5 (C)  </Text>
                <Text style={styles.text23}>Reporting and monitoring arrangements</Text>
                </View>
                <View style={styles.check1}>
                <View style={styles.check21}>
                <Text style={styles.text21}>5 (C3):   </Text>
                <Text style={styles.text1}>Investment in computers/hardware (INR)</Text>
                </View>
                <TextInput placeholder="Total Cost (INR) per annum" style={styles.textInput}  returnKeyType="done"
                  keyboardType="numeric" onChangeText={(text) => this.setState({hardwareCost: text}) }/>


                  <View style={styles.check21}>
                  <Text style={styles.text21}>5 (C4):   </Text>
                  <Text style={styles.text1}>District Skill Gap Study (INR)</Text>
                  </View>
                <TextInput placeholder="Total Cost (INR) per annum" style={styles.textInput}  returnKeyType="done"
                  keyboardType="numeric" onChangeText={(text) => this.setState({districtGap: text}) }/>

                <Text style={styles.text1}>    Total Cost (INR) – Annual </Text>
                <View style={styles.textBorder}>
                <Text style={[styles.submitText1]}>{result}</Text>
                </View>
                <Text   style={{marginTop: 30, color: "#ffffff",fontSize:18,alignSelf: "center",}}>( 4 / 11 )</Text>

                <View style= {styles.check1234}>
                <TouchableHighlight
                style={styles.back}
                onPress={() => this.props.navigation.navigate("Section5C2")}
                underlayColor='#fff'>
