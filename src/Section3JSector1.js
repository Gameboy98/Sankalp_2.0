...
export default class Section3JSector1 extends Component {
  ...
  async componentDidMount() {
    ...
    let sector1 =  await AsyncStorage.getItem("SelectedSectorB1");
    let sector2 =  await AsyncStorage.getItem("SelectedSectorB2");
    let sector3 =  await AsyncStorage.getItem("SelectedSectorB3");
    let sector4 =  await AsyncStorage.getItem("SelectedSectorB4");
    let sector5 =  await AsyncStorage.getItem("SelectedSectorB5");

    let sectorA1 =  await AsyncStorage.getItem("SelectedSector1");
    let sectorA2 =  await AsyncStorage.getItem("SelectedSector2");
    let sectorA3 =  await AsyncStorage.getItem("SelectedSector3");
    let sectorA4 =  await AsyncStorage.getItem("SelectedSector4");
    let sectorA5 =  await AsyncStorage.getItem("SelectedSector5");
    ...
    this.setState({sectorA1:sectorA1});
    this.setState({sectorA2:sectorA2});
    this.setState({sectorA3:sectorA3});
    this.setState({sectorA4:sectorA4});
    this.setState({sectorA5:sectorA5});
    this.fetchData();
  ...
  CheckTextInput = () => {
    if (this.state.populationTotalA == '' || this.state.populationFemaleA == '' || this.state.populationCasteA == ''|| this.state.populationTribeA == ''|| this.state.populationPwdA == '') {
      ...
      if(this.state.sector2=='' || this.state.sector2==null){
        this.props.navigation.navigate("Section3KSector1");
      }else {
        this.props.navigation.navigate("Section3JSector2");
      }
    }
  ...
  CheckSector = () => {
    if (this.state.sectorA2==null ) {
      this.props.navigation.navigate("Section3I");
    }else if(this.state.sectorA3==null) {
      this.props.navigation.navigate("Section3ISector2");
    }else if(this.state.sectorA4==null) {
      this.props.navigation.navigate("Section3ISector3");
    }else if(this.state.sectorA5==null) {
      this.props.navigation.navigate("Section3ISector4");
    }else {
      this.props.navigation.navigate("Section3ISector5");
    }
      ...
      <View>
        <View style={styles.question}>
          <View style={styles.check1}>
            ...
            <Text style={styles.text21}>3 (J) </Text>
            <Text style={styles.text1}>Placement Numbers for Previous FY (Demand-Supply Matching)</Text>
            </View>
            </View>
            <View style={styles.check21}>
            <Text style={styles.text21}>Sector 1:   </Text>
            <Text style={styles.text1}>{this.state.sector1}</Text>
            </View>
          ...
          <View style= {styles.check1}>
            <Text   style={{marginTop: 40,marginBottom: 10, color: "#ffffff",fontSize:18,alignSelf: "center",}}>( 11 / 20 )</Text>

            <View style= {styles.check1234}>
            <TouchableOpacity
            style={styles.back}
            onPress={this.CheckSector}
            underlayColor='#fff'>
