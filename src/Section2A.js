...
export default class Section2A extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      populationTotal2A: "",
      populationFemale2A: "",
      populationCaste2A: "-999",
      populationTribe2A: "-999",
      populationPwd2A: "",
...
async storeData() {
  try {
      await AsyncStorage.setItem("Screen2A1",this.state.populationTotal2A);
      await AsyncStorage.setItem("Screen2A2",this.state.populationFemale2A);
      await AsyncStorage.setItem("Screen2A3",this.state.populationCaste2A);
      await AsyncStorage.setItem("Screen2A4",this.state.populationTribe2A);
      await AsyncStorage.setItem("Screen2A5",this.state.populationPwd2A);
  } catch (error) {
  ...
  let url = "https://youth-apicalls.appspot.com/sankalp/add/demographic/section/two/";
    ...
    const data = await response.json();
     if(data.response.confirmation == 1){
         ...
         this.props.navigation.navigate("Section2B");
      }else {
...
async fetchSavedData() {
  ...
  CheckTextInput = () => {
    var total = parseInt(this.state.populationTotal2A, 10 );
    var female = parseInt(this.state.populationFemale2A, 10 );
    var sc = parseInt(this.state.populationCaste2A, 10 );
    var st = parseInt(this.state.populationTribe2A, 10 );
    var pwd = parseInt(this.state.populationPwd2A, 10 );

    if(this.state.populationTotal2A == '' || this.state.populationFemale2A == '' || this.state.populationCaste2A == '' || this.state.populationTribe2A == '' || this.state.populationPwd2A == ''){
          ...
          <View>
            <View style={styles.question}>
              <View style={styles.check1}>
                ...
                <Text style={styles.text21}>2 (A) </Text>
                <Text style={styles.text1}>Work Participation Rate (15 to 60 years)</Text>
                </View>
                </View>
                <Text style={styles.text10}>Overall</Text>
                <TextInput placeholder="Overall" style={styles.textInput}  returnKeyType="done"
                keyboardType="numeric" value={this.state.populationTotal2A} onChangeText={(text) => this.setState({populationTotal2A: text}) } />
               <Text style={styles.text10}>Female</Text>
              <TextInput placeholder="Female" style={styles.textInput}  returnKeyType="done"
                keyboardType="numeric"
                value={this.state.populationFemale2A}
                onChangeText={(text) => this.setState({populationFemale2A: text}) }/>
              ...
              <View style={styles.na}>
                {(this.state.status==0)?
                  <TextInput placeholder="SC" style={styles.textInput1}  returnKeyType="done"
                    keyboardType="numeric"
                    value={this.state.populationCaste2A}
                ...
                {(this.state.status2==0)?
                  <TextInput placeholder="ST" style={styles.textInput1}  returnKeyType="done"
                    keyboardType="numeric"
                    value={this.state.populationTribe2A}
              ...
              <TextInput placeholder="PWD" style={styles.textInput}  returnKeyType="done"
                keyboardType="numeric"
                value={this.state.populationPwd2A}
                onChangeText={(text) => this.setState({populationPwd2A: text}) }/>


              </View>
                <Text   style={{marginTop: 30, color: "#ffffff",fontSize:18,alignSelf: "center",}}>( 1 / 4 )</Text>
