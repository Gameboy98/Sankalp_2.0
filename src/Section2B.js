...
export default class Section2B extends Component {
  constructor(props) {
    super(props);
    this.state = {
...
toggleStatus2(index,value){
  ...
  async storeData() {
    try {
        await AsyncStorage.setItem("Screen2B1",this.state.populationTotal2B);
        await AsyncStorage.setItem("Screen2B2",this.state.populationFemale2B);
        await AsyncStorage.setItem("Screen2B3",this.state.populationCaste2B);
        await AsyncStorage.setItem("Screen2B4",this.state.populationTribe2B);
        await AsyncStorage.setItem("Screen2B5",this.state.populationPwd2B);
    } catch (error) {
    ...
    let url = "https://youth-apicalls.appspot.com/sankalp/add/demographic/section/two/";
      ...
      const data = await response.json();
       if(data.response.confirmation == 1){
           ...
           this.props.navigation.navigate("Section2C");
        }else {
        alert("Somethin Went Wrong");
        }
  }
  ...
  CheckTextInput = () => {
    var total = parseInt(this.state.populationTotal2B, 10 );
    var female = parseInt(this.state.populationFemale2B, 10 );
    var sc = parseInt(this.state.populationCaste2B, 10 );
    var st = parseInt(this.state.populationTribe2B, 10 );
    var pwd = parseInt(this.state.populationPwd2B, 10 );

    if(this.state.populationTotal2B == '' || this.state.populationFemale2B == '' || this.state.populationCaste2B == '' || this.state.populationTribe2B == '' || this.state.populationPwd2B == ''){
          ...
          <View>
            <View style={styles.question}>
              <View style={styles.check1}>
                ...
                <Text style={styles.text21}>2 (B) </Text>
                <Text style={styles.text1}>Unemployment Rate (15 to 60 years)</Text>
                </View>
                </View>
                <Text style={styles.text10}>Overall</Text>
                <TextInput placeholder="Overall" style={styles.textInput}  returnKeyType="done"
                keyboardType="numeric"
                value={this.state.populationTotal2B}
                onChangeText={(text) => this.setState({populationTotal2B: text}) } />
                <Text style={styles.text10}>Female</Text>
              <TextInput placeholder="Female" style={styles.textInput}  returnKeyType="done"
                keyboardType="numeric"
                value={this.state.populationFemale2B}
                onChangeText={(text) => this.setState({populationFemale2B: text}) }/>
                ...
                {(this.state.status==0)?
                  <TextInput placeholder="SC" style={styles.textInput1}  returnKeyType="done"
                    keyboardType="numeric"
                    value={this.state.populationCaste2B}
                ...
                {(this.state.status2==0)?
                  <TextInput placeholder="ST" style={styles.textInput1}  returnKeyType="done"
                    keyboardType="numeric"
                    value={this.state.populationTribe2B}
              ...
              <TextInput placeholder="PWD" style={styles.textInput}  returnKeyType="done"
                keyboardType="numeric"
                value={this.state.populationPwd2B}
                onChangeText={(text) => this.setState({populationPwd2B: text}) }/>

              </View>
                <Text   style={{marginTop: 30, color: "#ffffff",fontSize:18,alignSelf: "center",}}>( 2 / 4 )</Text>
