...
export default class Section2D extends Component {
  constructor(props) {
    super(props);
    this.state = {
      populationTotal2A: "",
  ...
  async componentDidMount() {
    ...
    let var11 =  await AsyncStorage.getItem("Screen2C1");
    let var12 =  await AsyncStorage.getItem("Screen2C2");
    let var13 =  await AsyncStorage.getItem("Screen2C3");
    let var14 =  await AsyncStorage.getItem("Screen2C4");
    let var15 =  await AsyncStorage.getItem("Screen2C5");
    ...
    this.setState({populationTotal2C:var11});
    this.setState({populationFemale2C:var12});
    this.setState({populationCaste2C:var13});
    this.setState({populationTribe2C:var14});
    this.setState({populationPwd2C:var15});



    let var19 =  await AsyncStorage.getItem("userId");
    this.setState({userId:var19});
    this.fetchSavedData();
}
  ...
  async storeData() {
    try {
        await AsyncStorage.setItem("Screen2D1",this.state.populationTotal2D);
        await AsyncStorage.setItem("Screen2D2",this.state.populationFemale2D);
        await AsyncStorage.setItem("Screen2D3",this.state.populationCaste2D);
        await AsyncStorage.setItem("Screen2D4",this.state.populationTribe2D);
        await AsyncStorage.setItem("Screen2D5",this.state.populationPwd2D);
    } catch (error) {
  ...
  CheckTextInput = () => {
    var total = parseInt(this.state.populationTotal2D, 10 );
    var female = parseInt(this.state.populationFemale2D, 10 );
    var sc = parseInt(this.state.populationCaste2D, 10 );
    var st = parseInt(this.state.populationTribe2D, 10 );
    var pwd = parseInt(this.state.populationPwd2D, 10 );

    if(this.state.populationTotal2D == '' || this.state.populationFemale2D == '' || this.state.populationCaste2D == '' || this.state.populationTribe2D == '' || this.state.populationPwd2D == ''){
          ...
          <View>
            <View style={styles.question}>
              ...
              <TextInput placeholder="Female" style={styles.textInput}  returnKeyType="done"
                ...
                {(this.state.status2==0)?
                  <TextInput placeholder="ST" style={styles.textInput1}  returnKeyType="done"
                    keyboardType="numeric"
                    value={this.state.populationTribe2D}
              ...
              <TextInput placeholder="PWD" style={styles.textInput}  returnKeyType="done"
                keyboardType="numeric"
                value={this.state.populationPwd2D}
                onChangeText={(text) => this.setState({populationPwd2D: text}) }/>

              </View>
                <Text   style={{marginTop: 30, color: "#ffffff",fontSize:18,alignSelf: "center",}}>( 4 / 4 )</Text>

                <View style= {styles.check1234}>
                <TouchableOpacity
                style={styles.back}
                onPress={() => this.props.navigation.navigate("Section2C")}
                underlayColor='#fff'>
