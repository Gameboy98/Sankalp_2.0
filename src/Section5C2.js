...
export default class Section5C2 extends Component {
  ...
  CheckTextInput = () => {
    if (this.state.dataUnits == '' || this.state.datacostAnnual == '') {
      alert('Please Enter Fields');
    }else {
      var f1 = this.state.costAnnum;
        var f2 = this.state.units ;
            const result = f1 && f2 ?  f2 * f1 : null;
        this.setState({dataTotal:result});
        this.props.navigation.navigate("Section5C3",{totalCost5C1:this.state.totalCost5C1,totalCost5C2:result,});
    ...
    let url = "https://youth-apicalls.appspot.com/sankalp/add/demographic/data/section/five/";
      ...
      console.log("SomeLog",data);
       if(data.response.confirmation == 1){
           this.props.navigation.navigate("Section5C3");
        }else {
        alert("Somethin Went Wrong");
        }
  }
  ...
  render() {
    var f1 = this.state.datacostAnnual;
      var f2 = this.state.dataUnits ;
          const result = f1 && f2 ?  f2 * f1 : null;
    return (
          ...
          <View>
            <View style={styles.question}>
              <View style={styles.check1}>
                ...
                <Text style={styles.text21}>5 (C2):   </Text>
                <Text style={styles.text1}>Hiring of resources for data collection & collation</Text>
                </View>

                <TextInput placeholder="Unit Cost (INR) per annum" style={styles.textInput}  returnKeyType="done"
                  keyboardType="numeric" onChangeText={(text) => this.setState({datacostAnnual: text}) }/>

                <TextInput placeholder="Units" style={styles.textInput}  returnKeyType="done"
                  keyboardType="numeric" onChangeText={(text) => this.setState({dataUnits: text}) }/>
                <Text style={styles.text1}>    Total Cost (INR) – Annual </Text>
                <View style={styles.textBorder}>
                <Text style={[styles.submitText1]}>{result}</Text>
                </View>
                <Text   style={{marginTop: 30, color: "#ffffff",fontSize:18,alignSelf: "center",}}>( 3 / 11 )</Text>

                <View style= {styles.check1234}>
                <TouchableHighlight
                style={styles.back}
                onPress={() => this.props.navigation.navigate("Section5C1")}
                underlayColor='#fff'>
